...
class FormatadorExcel:
    ...
    def salvar(self, abas: List[Tuple[str, pd.DataFrame]], log: List[str]) -> None:
        ...
        with pd.ExcelWriter(self.caminho_saida, engine="xlsxwriter") as writer:
            for nome, df in abas:
                _safe_to_excel(df, writer, nome)
                meta = self._metadata_df(df)
                meta_nome = f"_metadata_{nome}"
                _safe_to_excel(meta, writer, meta_nome)
            if log:
                df_log = pd.DataFrame({"log": log})
                _safe_to_excel(df_log, writer, "Log")
                _safe_to_excel(self._metadata_df(df_log), writer, "_metadata_Log")
...
def gera_relatorio_completo():
    ...
    try:
        ...
        abas_para_salvar: List[Tuple[str, pd.DataFrame]] = [
            ...
            ('Rec_Honorarios', df_honorarios),
            ('Rec_Certificados', df_certificados),
            ('Rec_Financeiras', df_financeiras),
            ('Desp_Base_Original', df_despesas),
            ('Desp_Operacionais', df_despesas_operacionais),
            ('Desp_Rateadas', df_desp_rateado_geral),
            ('Ticket_Geral', df_ticket_geral),
            ('Ticket_Regime', df_ticket_regime),
            ('Ticket_Reg_Atividade', df_ticket_reg_ativ),
            ('Ticket_Clientes', df_ticket_clientes),
            ('Impacto_Retiradas', df_impacto_retiradas),
            ('Top_Despesas', df_top_desp),
            ('Res_Regime_Com_Peso', df_regime_peso_excel),
            ('Res_Segmento_Com_Peso', df_seg_peso_excel), 
            ('Res_Regime_Sem_PL', df_regime_peso_no_pl_excel),
            ('Res_Segmento_Sem_PL', df_seg_peso_no_pl_excel),
            ('DRE_Simples', dre),
            ('DRE_Margem_Contrib', df_margem_contribuicao),
            ('Margem_Regime', df_margem_regime),
            ('Margem_Segmento', df_margem_segmento),
            ('Margem_Cliente', df_margem_cliente),
            ('PE_Regime', df_ponto_equilibrio),
            ('ROI_Regime', df_roi_regime),
            ('Clientes_Deficit', df_clientes_deficitarios),
            ('Ranking_Rentab', df_ranking_rentabilidade),
            ...
            ('Absorcao_Custos', df_absorcao),
            ('Curva_ABC', df_curva_abc),
            ('Indice_Concentracao', df_concentracao),
            ('Divers_Regime', df_diversificacao),
            ('Custo_Departamento', df_custo_dept),
            ('Ticket_Pond_Complex', df_ticket_ponderado),
            ('Cenario_Otimista', df_cenario_otim),
            ('Cenario_Pessimista', df_cenario_pess),
            ('Cenario_Reajuste', df_cenario_reaj),
            ('Cenario_Ideal_Ret', df_cenario_ideal),
        ]
